...
```
We first compute the global cost of each storm in a new attribute, `eco_data$overall`.
```{r}
eco_data$overall<-0
for (i in 1:length(eco_data[,1])){
        if (toupper(eco_data[i,3])=="M"){
                eco_data$overall[i]<-sum(eco_data$overall[i],(eco_data$PROPDMG[i]*10^6),na.rm=TRUE)
        }else if(toupper(eco_data[i,3])=="K"){
                eco_data$overall[i]<-sum(eco_data$overall[i],(eco_data$PROPDMG[i]*10^3),na.rm=TRUE)
        }else if(toupper(eco_data[i,3])=="B"){
                eco_data$overall[i]<-sum(eco_data$overall[i],(eco_data$PROPDMG[i]*10^9),na.rm=TRUE)
        }else{eco_data$overall[i]<-sum(eco_data$overall[i],(eco_data$PROPDMG[i]),na.rm=TRUE)}
}
```
The observations can be sorted down the overall cost (biggest first). We compute a new Pareto analysis for overall costs on an aggregation of the data, `agg_eco`.
```{r}
